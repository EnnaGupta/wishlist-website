(function () {
  ...
  function amazonPrice() {
    ...
    // 2. Try input fields that often contain price data
    ...
    // 4. Last resort: find price-whole + price-fraction
    const whole = document.querySelector('.a-price-whole')
    const fraction = document.querySelector('.a-price-fraction')
    if (whole?.textContent) {
      const w = whole.textContent.replace(/[^\d]/g, '')
      const f = fraction?.textContent?.replace(/[^\d]/g, '') || '00'
      const combined = parseFloat(`${w}.${f}`)
